...
int main(int argc, char* argv[]) {
	...
	int ADDR_LSB_BITS_ARR[6] = {9, 10, 11, 12, 13, 16};

	int BR_HIST_SFT_REG_WIDTH_ARR[6] = {10, 11, 12, 12, 13, 15};

	int STCNTR_WIDTH=2;
	const string files[4] = {"DIST-FP-1", "DIST-INT-1", "DIST-MM-1", "DIST-SERV-1"};
	ofstream output_file;
	output_file.open("results_combined_local.csv");
...
int main_analysis(int argc, char* argv[]) {
	...
	output_file[6]->open("results_combined_local.csv");
	...
	return 0;*/
	for (int i = 0; i < 7; ++i) {
		*output_file[i] <<"BUDGET"<<","<< "COST" << "," << "COST in K" << ","<< "ADDR_LSB_BITS" << "," << "BR_HIST_SFT_REG_WIDTH" << "," << "STCNTR_WIDTH";
		for (int f = 0; f < 4; ++f) {
	...
	/*
	for (int b = 0; b < 6	; ++b) {
		for (STCNTR_WIDTH = 2; STCNTR_WIDTH <= 3; ++STCNTR_WIDTH) {
			for (ADDR_LSB_BITS = 8; ADDR_LSB_BITS < 20; ++ADDR_LSB_BITS) {
				int prev_cost = 0;
				for (BR_HIST_SFT_REG_WIDTH = 1; BR_HIST_SFT_REG_WIDTH < 20; ++BR_HIST_SFT_REG_WIDTH) {
					Predictor t(NULL, ADDR_LSB_BITS, BR_HIST_SFT_REG_WIDTH, STCNTR_WIDTH);
					int cost = t.getcost();
					...
					if(cost <= budget[b]){
						...
						BR_HIST_SFT_REG_WIDTH--;
						Predictor p(NULL, ADDR_LSB_BITS, BR_HIST_SFT_REG_WIDTH, STCNTR_WIDTH);
						cost = p.getcost();
						printf("cost=%d\n",cost);
						printf("ADDR_LSB_BITS=%d BR_HIST_SFT_REG_WIDTH=%d\n",ADDR_LSB_BITS, BR_HIST_SFT_REG_WIDTH);

	 *output_file[6] << b << ","<< p.getcost() << "," << p.getcost()/1024 << ","<< ADDR_LSB_BITS << "," << BR_HIST_SFT_REG_WIDTH << "," << STCNTR_WIDTH;
						for (int i = 0; i < 4; ++i) {
							main_predictor(files[i], output_file[6], ADDR_LSB_BITS, BR_HIST_SFT_REG_WIDTH, STCNTR_WIDTH);
						}
	 *output_file[6] << endl;
	...
	for (int b = 0; b < 1	; ++b) {
		for (int STCNTR_WIDTH = 2; STCNTR_WIDTH <= 2; ++STCNTR_WIDTH) {
			for (int ADDR_LSB_BITS = 9; ADDR_LSB_BITS <= 19; ++ADDR_LSB_BITS) {
				int prev_cost = 0;
	...
	for (int b = 0; b < 1	; ++b) {
		for (int STCNTR_WIDTH = 2; STCNTR_WIDTH <= 2; ++STCNTR_WIDTH) {
			for (int ADDR_LSB_BITS = 19; ADDR_LSB_BITS <= 19; ++ADDR_LSB_BITS) {
				int prev_cost = 0;
				for (int BR_HIST_SFT_REG_WIDTH = 9; BR_HIST_SFT_REG_WIDTH <= 19; BR_HIST_SFT_REG_WIDTH++) {
					Predictor p(NULL, ADDR_LSB_BITS, BR_HIST_SFT_REG_WIDTH, STCNTR_WIDTH);
					int cost = p.getcost();
					printf("cost=%d\n",cost);
					printf("ADDR_LSB_BITS=%d BR_HIST_SFT_REG_WIDTH=%d\n",ADDR_LSB_BITS, BR_HIST_SFT_REG_WIDTH);
...
int main_predictor(string filename, ofstream* write, int ADDR_LSB_BITS, int BR_HIST_SFT_REG_WIDTH, int STCNTR_WIDTH){
	...
	Predictor predictor(&trace, ADDR_LSB_BITS, BR_HIST_SFT_REG_WIDTH, STCNTR_WIDTH);
